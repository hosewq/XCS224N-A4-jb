...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0].permute(1, 0, 2) # enc_hiddens = pad_packed_sequence(enc_hiddens, batch_first=True)[0]

        h_enc_cat = torch.cat([last_hidden[0], last_hidden[1]], dim=1)
        init_decoder_hidden = self.h_projection(h_enc_cat)

        c_enc_cat = torch.cat([last_cell[0], last_cell[1]], dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        unsqueezed_dec_hidden = torch.unsqueeze(dec_hidden, dim=2)
